...
speed.baler<-25 #highway travel speed                                       Source: placeholder
prod.baler.tons<-20 #dry tons per hour                                      Source: Darr

## Bale Collection Equipment ##
labor.collect<-18 #hourly wage for bale collection employees

# Tractor to pull bale collection wagon
price.wagontractor<-42 #per hour rental rate                                 Source: Darr
fuel.wagontractor<-10 #gallons consumed per hour                             Source: Darr

# Collection wagon
price.wagon<-80000 #initial cost                                            Source: Darr
life.wagon<-6 #expected life                                                Source: Darr
c1.wagon<-0 #depreciation constant                                          Source: placeholder
c2.wagon<-0 #depreciation constant                                          Source: placeholder
c3.wagon<-0 #depreciation constant                                          Source: placeholder
rc1.wagon<-0 #repair cost factor                                            Source: placeholder
rc2.wagon<-0 #repair cost factor                                            Source: placeholder
speed.wagon<-25 #highway travel speed                                       Source: placeholder
prod.wagon.tons<-25 #dry tons per hour                                      Source: Darr
...
for(i in 1:teams){
  ...
  cost.baler[[i]][1,"String"]<-sum(team.stats[[1]][,"CollectibleStover"])*price.string/kgperton
  cost.baler[[i]][1,"Overhead"]<-(cost.baler[[i]][1,"TractorRental"]+cost.baler[[i]][1,"BalerCapitalRecovery"]+cost.baler[[i]][1,"BalerRepair"]+cost.baler[[i]][1,"Fuel"]+cost.baler[[i]][1,"Lube"]+cost.baler[[i]][1,"Labor"]+cost.baler[[i]][1,"String"])*0.2
  cost.baler[[i]][1,"TotalCosts"]<-cost.baler[[i]][1,"TractorRental"]+cost.baler[[i]][1,"BalerCapitalRecovery"]+cost.baler[[i]][1,"BalerRepair"]+cost.baler[[i]][1,"Fuel"]+cost.baler[[i]][1,"Lube"]+cost.baler[[i]][1,"Labor"]+cost.baler[[i]][1,"String"]+cost.baler[[i]][1,"Overhead"]
  }
...
for(i in 1:teams){
  ...
  windrow.hours<-windrow.hours+cost.windrow[[i]][1,"WindrowHours"]
}
...
windrow.cost.hourly.windrow<-windrow.cost.total.windrow/windrow.hours
...
summary.windrow["Windrower","$/hr"]<-windrow.cost.total.windrow/windrow.hours
...
```{r Average Baler Costs}
#average costs per hour for utility tractor
total.hours.baler<-0
baler.hours<-0
for(i in 1:teams){
  total.hours.baler<-total.hours.baler+cost.baler[[i]][1,"TotalHours"]
  baler.hours<-baler.hours+cost.baler[[i]][1,"BalerHours"]
}

baler.cost.total.tractor<-0
baler.cost.total.labor<-0
baler.cost.total.fuel<-0
baler.cost.total.baler<-0
baler.cost.total<-0
for(i in 1:teams){
  baler.cost.total.tractor<-baler.cost.total.tractor+cost.baler[[i]][1,"TractorRental"]
  baler.cost.total.labor<-baler.cost.total.labor+cost.baler[[i]][1,"Labor"]
  baler.cost.total.fuel<-baler.cost.total.fuel+cost.baler[[i]][1,"Fuel"]+cost.baler[[i]][1,"Lube"]
  baler.cost.total.baler<-baler.cost.total.baler+cost.baler[[i]][1,"BalerCapitalRecovery"]+cost.baler[[i]][1,"BalerRepair"]
  baler.cost.total<-baler.cost.total+cost.baler[[i]][1,"TotalCosts"]
}

baler.cost.hourly.tractor<-baler.cost.total.tractor/total.hours.baler
baler.cost.hourly.labor<-baler.cost.total.labor/total.hours.baler
baler.cost.hourly.fuel<-baler.cost.total.fuel/total.hours.baler
baler.cost.hourly.baler<-baler.cost.total.baler/baler.hours
baler.cost.hourly.total<-baler.cost.total/total.hours.baler

#Setting up summary table
summary.baler<-matrix(data=0,ncol=2,nrow=6)
colnames(summary.baler)<-c("$/hr","$/ton")
rownames(summary.baler)<-c("Tractor","Labor","Fuel","Baler","Overhead","Total")
summary.baler<-as.data.frame(summary.baler)
summary.baler["Tractor","$/hr"]<-baler.cost.total.tractor/total.hours.baler
summary.baler["Labor","$/hr"]<-baler.cost.total.labor/total.hours.baler
summary.baler["Fuel","$/hr"]<-baler.cost.total.fuel/total.hours.baler
summary.baler["Baler","$/hr"]<-baler.cost.total.baler/baler.hours
summary.baler["Tractor","$/ton"]<-baler.cost.total.tractor/(feedstock.requirement/kgperton)
summary.baler["Labor","$/ton"]<-baler.cost.total.labor/(feedstock.requirement/kgperton)
summary.baler["Fuel","$/ton"]<-baler.cost.total.fuel/(feedstock.requirement/kgperton)
summary.baler["Baler","$/ton"]<-baler.cost.total.baler/(feedstock.requirement/kgperton)
summary.baler["Overhead","$/ton"]<-(summary.baler["Tractor","$/ton"]+summary.baler["Labor","$/ton"]+summary.baler["Fuel","$/ton"]+summary.baler["Baler","$/ton"])*0.2
summary.baler["Total","$/ton"]<-sum(summary.baler[1:5,"$/ton"])
summary.baler
```
