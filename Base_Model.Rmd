...
The stover collection process cannot begin until the corn grain harvests have taken place.  Since harvest completion will vary from farm to farm, each farm's availability is provided on a random date.  The percentage of farms which have completed their harvesting are given by the function estimated by Darr:

harvest (%) = 1-exp(-0.00000598018*(days since August 15)^3.36817)

This function was calculated from data obtained from NASS's weekly reports of working days and percent of corn harvested for the state of Iowa from 1999-2010.  The dates considered were from the middle of August until the end of the reporting period which is when the corn harvest has reached 95% completion.
...
Here is where I want code that requires two inputs: teams and start date.

```{r Number of harvest teams}
teams<-10 #select this
cutoff<-360/teams
```

```{r Division of farms into teams}
part.farms$theta<-atan2(part.farms$YCoord,part.farms$XCoord)*180/pi
for(i in 1:nrow(part.farms)){
  if(part.farms[i,12]<0){part.farms[i,12]<-part.farms[i,12]+360}
}

farm.assignments<-replicate(teams,data.frame())
for(i in 1:teams){
farm.assignments[[i]]<-subset(part.farms,(i-1)*cutoff<part.farms$theta & part.farms$theta<i*cutoff)
}

#Harvestable farms only graphic
pp=ggplot(circle,aes(x,y)) 
for(i in 1:(teams)){
  pp=pp + geom_path() + geom_point(data=part.farms,x=part.farms$XCoord,y=part.farms$YCoord,col="red") + geom_point(data=refine.locations,x=refine.locations$XCoord,y=refine.locations$YCoord,col="blue",size=8)+ ggtitle("Farm Locations") + theme(plot.title = element_text(lineheight=.8, face="bold"))+xlab("Miles (West to East)")+ylab("Miles (North to South)") + geom_segment(x=0,y=0,xend=r*cos((i-1)*cutoff*pi/180),yend=r*sin((i-1)*cutoff*pi/180))}
print(pp)
```

```{r Determining whether a farm is ready or not}
#Creating column in each data frame for indicator function of whether a farm is ready or not
for(i in 1:teams){
  farm.assignments[[i]]$Ready<-0
}
...
```{r Choosing a start date}
start<-20 #select this
harvest.percent(start)
```
...
for(i in 1:teams){
  for(j in 1:nrow(farm.assignments[[i]])){
  if(farm.assignments[[i]][j,1] %in% ready[[i]]){farm.assignments[[i]][j,13]<-1}
  }
}
...
notready.farms<-subset(part.farms,select=c("ID","XCoord","YCoord","Distance","AvgYield","AvgStover","AcresHarvested","Stover","WindrowTime"))
...
ready.farms<-subset(notready.farms,notready.farms$ready==1)
...
for(i in 1:nrow(notready.farms)){
  if(notready.farms[i,1] %in% ready){notready.farms[i,10]<-1}
}
...
```{r Daylight available}
#Importing data from http://www.sunearthtools.com/solar/sunrise-sunset-calendar.php#form using Clarion, IA
d<-"/Users/goodrich/Desktop/R Projects/Supply.Chain.Development/DayLength.csv"
daylight.data<-read.table(file=d, header=TRUE,sep=",") 

#converting values to numeric
daylight.data$Hours<-as.numeric(as.character(daylight.data$Hours))
```

Step 1: 20' windrower/shredder shreds stalks and discharges material into side windrow.  This type of machine is selected because shredders have less opportunity to contact the ground and hence bales will have a lower level of soil contamination than with a raking technique.  With a side windrow there is also the ability to push windrows together and eliminate baling trips.  

Equipment: 
John Deere 5075E Cab Utility Tractor (57 pto hp): $40769 list price (cite: Build Your Own John Deere)
Hiniker 5620 Windrower: $42524 list price (cite: http://generalimp.com/sites/default/files/product-pdf/hiniker_shredders_windrower_0.pdf)

Specifications:
Windrower manual states not to exceed 25 mph while transporting along the highway.  Windrower manual also states that recommended speeds are 3-7 mph.
