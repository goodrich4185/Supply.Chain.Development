...
<<Calculating Iowa annual statewide average yield, echo=FALSE>>=
remove(list=ls())
...
<<Finding prior values for lognormal prior parameters,tidy=FALSE,echo=FALSE,error=FALSE,message=FALSE>>=
...
<<Sampler using multinomial model for data>>=
...
model{
  for(i in 1:6){
  y[i]~dmulti(p[i],n[i])
  p[i]<-plnorm(u[i],mu,1/sigma2)-plnorm(l[i],mu,1/sigma2)
  }

mu~dnorm(log(155.8869),log(50))
sigma2~dgamma(2,log(3.428357))
}"
u<-c(25,100,250,500,1000,10^6)
l=c(0,25,100,250,500,1000)
n=c(504,13122,13962,10149,5779,2079)
data<-list(n=n,y=c(1,2,3,4,5,6),l=l,u=u)
m=jags.model(textConnection(model),data,quiet=TRUE)
@

<<Sampler,echo=FALSE,error=FALSE,message=FALSE>>=
require(rjags)
load.module("bugs")
area_dummy<-as.data.frame(c(rep(0,504),rep(1,13122),rep(2,13962),rep(3,10149),rep(4,5779),rep(5,2079)))
colnames(area_dummy)<-"dummy"
area_dummy$p<-rep(NA,nrow(area_dummy)) 
...
cred_int = function(p, prob=0.95) {p$log_mean+c(-1,1)*qt(1-(1-prob)/2,p$n-1)*sqrt(p$log_mean_var/p$n)}
cred_int(mean_moments)
...
<<Data simulation,echo=FALSE,message=FALSE,error=FALSE>>=
#radius calculation
Iowa_area=56271.55
r=(Iowa_area/pi)^(1/2)
